		...
		ChangeLangInterface
{
	...
	public synchronized void switchDirection()
	{
		...
		if ( this.tableViewSQLDirection == Orientation.HORIZONTAL )
		{
			...
			{
				List<Object> dataRow = this.getItems().get(i);
	...
	private void copyTable( int withHead )
	{
		...
		if ( this.tableViewSQLDirection == Orientation.HORIZONTAL )
		{
			...
			for ( Integer id : selectedIndices )
			{
				...
				for ( int i = 1; i < colSize; i++ )
				{
					...
					Object val = this.getItems().get( id ).get( i );
					if ( val != null )
					{
						sbTmp.append( val );
					}
		...
		else
		{
			...
			for ( int i = 0; i < rowSize; i++ )
			{
				...
				{
					Object val = this.getItems().get( i ).get( j );
	...
	private List<List<Object>> getDataList()
	{
		...
		if ( this.tableViewSQLDirection == Orientation.HORIZONTAL )
		{
			...
			for ( int i = 0; i < rowSize; i++ )
			{
				List<Object> dataRowOrg = this.getItems().get(i);
				List<Object> dataRow = new ArrayList<>();
